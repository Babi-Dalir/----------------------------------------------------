...
class Product extends Model
{
    ...
    public function submitedComments(){
    ...
    public static function createProduct($request)
    {
        $product= Product::query()->create([
            ...
            'image'=>ImageManager1::saveImage('products',$request->image),
        ]);
    ...
    public static function updateProduct($request,$id)
    {
        ...
        $product->update([
            ...
            'category_id'=>$request->input('category_id'),
            'brand_id'=>$request->input('brand_id'),
            'image'=>$request->image ? ImageManager1::saveImage('products',$request->image) : $product->image,
        ]);
        $product->tags()->sync($request->tags);
    }

    public static function sellerCreateProduct($request)
    {
        $product= Product::query()->create([
            'user_id'=>auth()->user()->id,
            'title'=>$request->input('title'),
        ...
        $product->tags()->attach($request->tags);
        ProductGuaranty::createProductGuaranty($request,$product->id);
    }
}
