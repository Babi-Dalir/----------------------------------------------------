...
class Payment extends Component
{
    ...
    public function checkDiscountCode()
    {
        ...
        if ($discount) {
            ...
            session()->flash('success_discount', "تخفیف با موفقیت ثبت شد");
        } else {
    ...
    public function checkGiftCartCode()
    {
        ...
        if ($gift_cart) {
            ...
            session()->flash('success_gift_cart', "کد هدیه با موفقیت ثبت شد");
        } else {
    ...
    {
        $selected_address=Address::query()->where('user_id',auth()->user()->id)
        ...
        return view('livewire.frontend.order.payment',compact('send_price','selected_address'));
    }
}
