...
class EnterOrExitVendorModal extends Component
{
    public $type = 'enter';
    ...
    public function enterOrExitInVendor($product_guaranty_id,$vendor_id)
    {
        $this->vendor_product = VendorProduct::query()
            ...
            ->where('product_guaranty_id',$product_guaranty_id)->first();
    ...
    public function submitEnterOrExit()
    {
        if ($this->type == VendorEventType::Enter->value){
            ...
            session()->flash('message',"تعداد اضافه شد");
            ...
            $this->emit('refreshVendorList');
            $this->reset('count','type');
            session()->flash('message',"تعداد کم شد");
        }
    }
    public function render()
    {
        return view('livewire.admin.vendor.enter-or-exit-vendor-modal');
    }
}
