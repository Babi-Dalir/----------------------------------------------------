...
class RegisteredUserController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $checkSend=VerificationCode::checkTowMinutes($request->mobile);
        if($checkSend){
            $code=rand('11111',99999);
            VerificationCode::createVerificationCode($request->mobile,$code);
            //send SMS
            $smsService= new SMSService();
            $smsService->setReceirver($request->mobile);
            $smsService->setContent($code);

            $messageService= new MessageService($smsService);
            $messageService->send();
        }else{
            return redirect()->back()->with('message','برای ارسال مجدد کدفعالسازی 2دقیقه صبر کنید');
        }
