<div class="table overflow-auto" tabindex="8">
    ...
    <div class="row m-1">
        <a href="{{ route('create.product.guaranties',$product_id) }}" class="btn btn-outline-skype">ایجاد تنوع قیمت</a>
    </div>
    <table class="table table-striped table-hover">
        ...
        <tr>
            ...
            <th class="text-center align-middle text-primary">قیمت نمایش داده</th>
        ...
        <tbody>
            @foreach ($product_guaranties as $index=>$product_guaranty )
            <tr>
                <td class="text-center align-middle">{{$product_guaranties->firstItem()+$index}}</td>
                ...
                <td class="text-center align-middle">
                    @if ($product_guaranty->spacial_start == null && $product_guaranty->special_expiration == null )
                            <span class="cursor-pointer badge badge-light">فروش عادی</span>

                    @else
                    <span class="cursor-pointer badge badge-danger">فروش فوق العاده</span>

                    @endif
                    </td>

                <td class="text-center align-middle">
                    <a class="btn btn-outline-info" href="{{ route('edit.product.guaranties',[$product_guaranty->id,$product_id]) }}">
                        ویرایش
                    </a>
                </td>
                <td class="text-center align-middle">
                ...
                <td class="text-center align-middle" @if(auth()->user()->is_admin) wire:click='chaneProductGuarantyStatus({{ $product_guaranty->id }})' @endif>
                    ...
                    @elseif ($product_guaranty->status==\App\Enums\ProductStatus::StopProduction->value)
                        <span class="cursor-pointer badge badge-dark">توقف تولید</span>
                ...
                <td class="text-center align-middle">{{\Hekmatinasser\Verta\Verta::instance($product_guaranty->created_at)->format(' %d,%B %Y')}}</td>
            </tr>
            @endforeach
    ...
    <div style="margin: 40px !important;"
         ...
         {{$product_guaranties->appends(Request::except('page'))->links()}}
    </div>
</div>
@section('scripts')
    <script>
