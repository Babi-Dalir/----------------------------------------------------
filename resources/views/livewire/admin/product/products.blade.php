    ...
    <div class="form-group row">
        ...
        <div class="col-sm-2">
            <a href="{{ route('products.trashed') }}" class="btn btn-outline-warning">
                <i class="ti-trash">محصولات حذف شده</i>
            </a>
        </div>
            <div class="col-sm-2">
                <a href="{{ route('products.create') }}" class="btn btn-outline-info">
                    <i class="ti-plus">اضافه کردن محصول </i>
                </a>
            </div>
        @else
            <div class="col-sm-2">
                <a href="{{ route('seller.create.product') }}" class="btn btn-outline-info">
                    <i class="ti-plus">اضافه کردن محصول توسط فروشنده</i>
                </a>
            </div>
        @endif
    ...
    <table class="table table-striped table-hover">
        ...
        @foreach($products as $index=>$product)
            <tr>
                ...
                <td class="text-center align-middle">
                    <a class="btn btn-outline-info" href="{{route('product.guaranties',$product->id)}}">
                        تنوع قیمت
                    </a>
                </td>
                <td class="text-center align-middle">
                ...
                @if(auth()->user()->is_admin)
                <td class="text-center align-middle">
                    <a class="btn btn-outline-dark" href="{{route('product.discussions',$product->id)}}">
                        نقدو بررسی
                    </a>
                </td>
                ...
                <td class="text-center align-middle">
                    @if(auth()->user()->id == $product->user->id | auth()->user()->is_admin)
